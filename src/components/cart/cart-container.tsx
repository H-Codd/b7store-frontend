...
import { ShippingBox } from "./cart-shipping-box";
  ...
  return (
    <div>
      <div className="flex items-center gap-2">
        <Image
          src={"/ui/shopping-bag-4-line-black.png"}
          alt=""
          width={24}
          height={24}
        />
        <div className="text-lg">
            Seu carrinho de compras <span className="text-gray-500">({cartStore.cart.length} {cartStore.cart.length != 1 ? 'itens' : 'item'})</span> </div>
      </div>
        ...
        <div className="flex-1 md:max-w-sm flex flex-col gap-4">
          ...
          <div className="bg-white border border-gray-200 rounded-sm">
            ...
            <div className="p-6">
              ...
              <div className="flex justify-end text-xs text-gray-500 mb-10">Em até 12x no cartão</div>

              <FinishPurchaseButton />
              <div className="text-center mt-6">
                <Link href={"/"} className="text-xs text-gray-500">Compra outros Produtos</Link>
              </div>
            </div>
          </div>
        </div>
