        ...
        <div className="mb-8">
            <div className="flex justify-between items-center border-b border-gray-200 pb-4">
                ...
                <div onClick={() => setOpened(!opened)} className="size-8 flex justify-center items-center cursor-pointer">
                    <Image src={'/ui/arrow-left-s-line.png'} alt="ArrowUp" width={24} height={24} className={`${opened ? 'rotate-0': 'rotate-180'} transition-all`}/>
                </div>
            </div>
            <div className={`overflow-hidden ${opened ? 'max-h-96' : 'max-h-0'} transition-all`}>
                {values.map(item => (
